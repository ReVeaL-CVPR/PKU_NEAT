...
params.OverallMutationRate = 0.35
params.CrossoverRate = 0.75
params.MultipointCrossoverRate = 0.4
params.PopulationSize = 16

genome = NEAT.Genome(0, 171, 0, 4, False, NEAT.ActivationFunction.UNSIGNED_SIGMOID, NEAT.ActivationFunction.UNSIGNED_SIGMOID, 0, params)

try:
    pop = NEAT.Population("weak.txt")
except:
    pop = NEAT.Population(genome, params, True, 1.0, random.randint(1,650034))
...
pool = Pool(8)
log = 0
#aaa = [pop.GetBestGenome(),pop4.GetBestGenome(),pop2.GetBestGenome(),pop3.GetBestGenome()]
#pool.map(evaluate, aaa)
#evaluate(pop.GetBestGenome())
for generation in range(10000): # run for 100 generations
    ...
    t = pool.map(evaluate, genome_list)

    for genome,fitness in zip(genome_list,t):
